...
void Log_LW (DWORD PC, DWORD VAddr)
{
	...
	{
		if ( VAddr >= 0xA0000000 && VAddr < (0xA0000000 + RdramSize)) 
			return;
		...
		if ( VAddr >= 0xB0000040 && ((VAddr - 0xB0000000) < RomFileSize)) 
			return;
...
void Log_SW (DWORD PC, DWORD VAddr, DWORD Value) 
{
    ...
    if ( VAddr == 0xA4080000)
	{
		if (!LogOptions.LogSPRegisters) 
			return;
		LogMessage("%08X: Writing 0x%08X to SP_PC",PC, Value );
		return;
	}
...
void StartLog (void) {
	...
	strcpy(logFileName, main_directory);
	strcat(logFileName, "Project64.log");
