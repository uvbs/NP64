...
char CurrentSave[256], main_directory[_MAX_PATH];
HMENU hMainMenu;
HINSTANCE hInst;
...
char * GetExtIniFileName(void) {
	...
	strcpy(IniFileName, main_directory);
	strcat(IniFileName, ExtIniName);
	return IniFileName;
}
...
char * GetLangFileName(void) {
	static char IniFileName[_MAX_PATH];
	strcpy(IniFileName, main_directory);
	strcat(IniFileName, LangFileName);
	return IniFileName;
}
...
char * GetNotesIniFileName(void) {
	static char IniFileName[_MAX_PATH];
	strcpy(IniFileName, main_directory);
	strcat(IniFileName, NotesIniName);
	return IniFileName;
}
...
LRESULT CALLBACK RomInfoProc (HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_INITDIALOG:
		...
		CRC = RomHeader[0x13];
		CRC = (CRC << 8) +  RomHeader[0x12];
		CRC = (CRC << 8) +  RomHeader[0x11];
		CRC = (CRC << 8) +  RomHeader[0x10];
		sprintf(&String[1],"0x%08X",CRC);
		SetDlgItemText(hDlg,IDC_INFO_CRC1,String);

		CRC = RomHeader[0x17];
		CRC = (CRC << 8) +  RomHeader[0x16];
		CRC = (CRC << 8) +  RomHeader[0x15];
		CRC = (CRC << 8) +  RomHeader[0x14];
		sprintf(&String[1],"0x%08X",CRC);
