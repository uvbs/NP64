...
void DoAddressError ( BOOL DelaySlot, DWORD BadVaddr, BOOL FromRead) {
	...
	DebugError("AddressError");
	if (( STATUS_REGISTER & STATUS_EXL  ) != 0 ) { 
		DebugError("EXL set in AddressError Exception");
	}
	if (( STATUS_REGISTER & STATUS_ERL  ) != 0 ) { 
		DebugError("ERL set in AddressError Exception");
	}
...
void DoBreakException ( BOOL DelaySlot) {
	...
	if (( STATUS_REGISTER & STATUS_EXL  ) != 0 ) { 
		DebugError("EXL set in Break Exception");
	}
	if (( STATUS_REGISTER & STATUS_ERL  ) != 0 ) { 
		DebugError("ERL set in Break Exception");
	}
...
void _fastcall DoSysCallException ( BOOL DelaySlot) {
	if (( STATUS_REGISTER & STATUS_EXL  ) != 0 ) { 
		DebugError("EXL set in SysCall Exception");
	}
	if (( STATUS_REGISTER & STATUS_ERL  ) != 0 ) { 
		DebugError("ERL set in SysCall Exception");
	}
